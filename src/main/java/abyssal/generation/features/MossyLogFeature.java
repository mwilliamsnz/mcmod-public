...
public class MossyLogFeature extends Feature<NoneFeatureConfiguration> {
    public MossyLogFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> ctx) {
        ...
        if (stumpRand < 0.1) {
            this.tryPlaceBlock(level, projectedOrigin, Blocks.BIRCH_LOG.defaultBlockState(), false);
            this.tryPlaceBlock(level, projectedOrigin.above(), ModBlocks.MOSSY_BIRCH.get().defaultBlockState(), false);
            projectedOrigin = projectedOrigin.above();
        } else if (stumpRand < 0.2) {
            this.tryPlaceBlock(level, projectedOrigin, Blocks.BIRCH_LOG.defaultBlockState(), false);
        } else if (stumpRand < 0.5) {
            this.tryPlaceBlock(level, projectedOrigin, ModBlocks.MOSSY_BIRCH.get().defaultBlockState(), false);
        }
        ...
        if(topTypeRand < 0.2 && stumpRand < 0.5) {
            ...
            this.tryPlaceBlock(level, projectedOrigin.above(), Blocks.BROWN_MUSHROOM.defaultBlockState(), false);
        }

        for(int i = 0; i < overSolid; i++) {
            if(random.nextFloat() < 0.80) {
                this.tryPlaceBlock(level, placePos, ModBlocks.MOSSY_BIRCH.get().defaultBlockState().setValue(RotatedPillarBlock.AXIS, dir.getAxis()), true);
                ...
                if(topTypeRand < 0.5) {
                    ...
                    this.tryPlaceBlock(level, placePos.above(), Blocks.BROWN_MUSHROOM.defaultBlockState(), false);
                }
            } else {
                this.tryPlaceBlock(level, placePos, Blocks.BIRCH_LOG.defaultBlockState().setValue(RotatedPillarBlock.AXIS, dir.getAxis()), true);
            }
